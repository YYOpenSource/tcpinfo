...
TcpInfo::TcpInfo()	
{
	...
	req.nlh.nlmsg_type = YY_DIAG_FAMILY_ID;
	req.nlh.nlmsg_flags = NLM_F_REQUEST;
	req.nlh.nlmsg_pid = 0;
	req.nlh.nlmsg_seq = 0;
	req.gnlh.cmd = YY_TCP_C_GETINFO;
	req.gnlh.version=0;	
...
void TcpInfo::print(struct yy_tcp_info& info){
	cout<<"{"<<dec
		<<"\"rcv_nxt\":"<<info.rcv_nxt
...
void TcpInfo::setSockToDump(struct yy_tcp_sockid & sockid){
	req.nlh.nlmsg_flags = NLM_F_REQUEST;
...
std::forward_list<struct yy_tcp_info> TcpInfo::getTcpInfo(struct yy_tcp_sockid & sockid){
	req.id = sockid;
	if(sockid.sport != 0){
		setSockToDump(sockid);
	}
	...
	*/
	...
	forward_list<struct yy_tcp_info> tcp_infos;
    while (NLMSG_OK(h, status)) {
		...
		struct yy_tcp_info* _yinfo = (struct yy_tcp_info*)(buf + sizeof(struct nlmsghdr));
		tcp_infos.push_front(*_yinfo);
		print(*_yinfo);
